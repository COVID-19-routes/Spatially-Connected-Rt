...
assert(all(string(pop.NUTS_3') == Provinces), ...
...
switch purpose
    ...
    case 'run_veneto'
        ...
        waitbar(0, wbarh, "computing spatially explicit Rt");
        [Rt1, diagnostic, model_out] = ...
            pf(Fp, par, Q, ResPop, csi, @waitbar);
    ...
    case 'compare_with_EpiEstim'
        ...
        Fv = smoothdata(cases, 'movmean', [13, 0]);
        Fv(Fv < 0) = 0;
        cases(cases < 0) = 0;
        ...
        figure('Renderer', 'painters', ...
            'Units', 'centimeters', ...
            'Position', [0, 0, 16, 9])
        a = find(R_SMC.Q95(1, :), 1, 'first');
        b = find(R_EE(1, :), 1, 'first');
        c = max(a, b);
        filler(Time(a:end), R_SMC.Q95(a:end), R_SMC.Q05(a:end), ...
            colors(1), 0.2);
        hold on
        p1 = plot(Time(a:end), R_SMC.Q50(a:end), ...
            'Color', colors(1), 'LineWidth', 0.75);
        p2 = plot(Time(b:end), R_EE(b:end), ...
    ...
    case 'make_figure_incidence'
        ...
        % Make figure
        colors = ["#fbcf36", "#11776c", "#5e4fa2", "#5299cb", "#98d5a4", ...
            "#ee6445", "#9d1642"];
        figure('Renderer', 'painters', ...
            'Units', 'centimeters', ...
            'Position', [0, 0, 11.4, 6])
        hold on
        for i = 1:7
            plot(Time(1:600), incidence(i, 1:600), ...
                'linewidth', 1, 'Color', colors(i))
        end
        ylabel(append('Incidence [', char(8240), ']'))
        box off
        legend( ...
            ["Verona (VR)", "Vicenza (VI)", "Belluno (BL)", ...
            "Treviso (TV)", "Venice (VE)", "Padua(PD)", "Rovigo(RO)"], ...
            'location', 'east')
        legend boxoff
